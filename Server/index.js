        ...
        app.post('/sell/add', async (req, res) => {
            ...
            async function deleteStock(){
                    ...
                    for(let i=0; i<data.data.length; i++){
                        console.log(data.data[i].barcode)
                        const barcode = data.data[i].barcode
                        barcodes.push(data.data[i].barcode)
                    }
                    
                    for (const element of barcodes) {
                        const delStock = await stock.deleteOne({"barCode": element});
                    }
                }
            try {
